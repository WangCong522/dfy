...
class memberControl extends BaseMemberControl
{
    ...
    public function homeOp(){
        ...
        $UserInfo['to_receive'] = array();


        //待领取奖品
        $UserInfo['to_receive'] = $goods_model->getToReceivePrizeInfo($UserInfo['begin_group'],$UserInfo['group_id']);
        //已经领取过得
        $member_model = Model('member');
        $BantPrizeInof = $goods_model->getBantPrizeInof($this->userid);

        foreach ($UserInfo['to_receive'] as $key => $value) {
            foreach ($BantPrizeInof as $k => $v) {
                if ($value['gid'] == $v['good_id']) {
                    unset($UserInfo['to_receive'][$key]);
                }
        ...
        if ($UserInfo['begin_group'] == 0) $UserInfo['begin_group'] = $UserInfo['group_id'];
