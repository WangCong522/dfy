...
function makeSeccode($nchash){
	...
	setBnCookie('seccode'.$nchash, encrypt(strtoupper($seccode)."\t".(time())."\t".$nchash,MD5_KEY),3600);
	return $seccode;
}
...
function checkSeccode($nchash,$value){
	...
	if (!$return) setBnCookie('seccode'.$nchash,'',-3600);
	return $return;
}
 ...
 */
function setBnCookie($name, $value, $expire='3600', $path='', $domain='', $secure=false){
	if (empty($path)) $path = '/';
...
function chksubmit($check_token = false, $check_captcha = false, $return_type = 'alert'){
	...
	// var_dump($_POST['form_submit']);exit;
	...
	if ($check_captcha){
		if (!checkSeccode($_POST['nchash'],$_POST['captcha'])){
		    setBnCookie('seccode'.$_POST['nchash'],'',-3600);
			if ($return_type == 'alert'){
				showDialog('验证码错误!');
			}else{
				return -12;
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('InShopBN') or exit('Access Invalid!'); return ".$data.";";
